...
const ProductManagement = ({ darkMode,  filterText}) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axios.get("https://inventoryapps-g67r.onrender.com/api/products/products", {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`https://inventoryapps-g67r.onrender.com/api/products/${currentProductId}`, productData);
      } else {
        await axios.post("https://inventoryapps-g67r.onrender.com/api/products/add-product", productData);
      }
      fetchProducts();
      handleClose();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://inventoryapps-g67r.onrender.com/api/products/${id}`);
      fetchProducts();
    } catch (error) {
